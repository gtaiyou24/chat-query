...
from modules.authority.domain.model.user import VerificationTokenGenerated, UserRepository, PasswordResetTokenGenerated
...
class PasswordForgotSubscriber(DomainEventSubscriber[PasswordResetTokenGenerated]):
    ...
    def handle_event(self, domain_event: PasswordResetTokenGenerated) -> None:
        ...
        try:
            self.send_mail_service.send(
                domain_event.email_address,
                '【Analytics GPT】パスワードのリセット',
                f"""<html>
                <body>
                <h1>パスワードをリセット</h1>
                <a href="{os.getenv('FRONTEND_URL')}/auth/new-password?token={domain_event.token.value}">
                    こちらをクリックしてください。
                </a>
                </body>
                </html>""")
